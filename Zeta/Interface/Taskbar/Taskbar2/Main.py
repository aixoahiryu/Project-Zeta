...
class Taskbar2(Toplevel):
	...
	def chdir(self, path):
		...
		if path!='' and (not os.path.exists(path+'/# META')): os.mkdir(path+'/# META')
		Button2(self.frame1, image=self.imgmenu, side='left', fill='y', geometry='top')
		Label(self.frame1, text='# [ ').pack(side='left', fill='y')
		Button2(self.frame1, text='META', side='left', fill='y', geometry='top', buffer=['Core', 'Vector', 'Tools', '|', 'Instinct'], path=path+'/# META/#META')
		Button2(self.frame1, text='Root', side='left', fill='y', geometry='top', buffer=['Requirement', 'Specification', '|', 'Idea', 'Methodology'], path=path+'/# META/#Root')
		Button2(self.frame1, text='Search', side='left', fill='y', geometry='top', buffer=['[ Engine ]', '|', 'Keyword', 'Variant'], path=path+'/# META/#Search')
		Button2(self.frame1, text='Link', side='left', fill='y', geometry='top', buffer=['Main', 'Repository', '|', 'Blackmarket'], path=path+'/# META/#Link')
		Label(self.frame1, text=' ]').pack(side='left', fill='y')
		Button2(self.frame1, text='[ Note ]', side='left', fill='y', geometry='top', buffer=['Internal', '|', 'Shared', 'People'], path=path+'/# META/[ Note ]')
		Button2(self.frame1, text='[ Reference ]', side='left', fill='y', geometry='top', buffer=['Documentation', 'Research', 'Case study', '|', 'Book'], path=path+'/# META/[ Reference ]')
		Button2(self.frame1, text='[ Network ]', side='left', fill='y', geometry='top', buffer=['Articles', 'Forums', 'Q & A', 'Tutorial', '|', 'Anonymous'], path=path+'/# META/[ Network ]')
		...
		Button2(self.frame2, image=self.imgmenu, command= lambda: Workspace.hide(Workspace.active), side='right', fill='y', geometry='top')
		btnairlock = Button2(self.frame2, text='Ω[ Airlock ]', side='right', fill='y', geometry='top', path=path+'/# META/Ω[ Airlock ]')
		btnscraps = Button2(self.frame2, text='Σ[ Scraps ]', side='right', fill='y', geometry='top', path=path+'/# META/Σ[ Scraps ]')
		Label(self.frame2, text=' ]').pack(side='right', fill='y')
		btnraw = Button2(self.frame2, text='Raw', side='right', fill='y', geometry='top', buffer=['History', 'Resource', 'Download'], path=path+'/# META/Δ[ Raw ]')
		btnfile = Button2(self.frame2, text='File', side='right', fill='y', geometry='top', buffer=['Scraps', 'Sandbox'], path=path+'/# META/Δ[ File ]')
		Label(self.frame2, text='Δ[ ').pack(side='right', fill='y')
		Button2(self.frame2, text=' Journal', image=self.imghdd, compound='left', side='right', fill='y', geometry='top', path=path+'/# META/Journal')
		Button2(self.frame2, image=self.imgplay, side='right', fill='y', geometry='top', buffer=['Mindset', 'Ambient'], path=path+'/# META/▷ Play')
		Button2(self.frame2, image=self.imgtext, side='right', fill='y', geometry='top', buffer=['Learn', 'Alternative'], path=path+'/# META/≡ Read')
		Button2(self.frame2, image=self.imgex, side='right', fill='y', geometry='top', buffer=['Project', 'Sticky'], path=path+'/# META/√ Experiment')
