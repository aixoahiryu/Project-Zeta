...
class Switcher(Window):
	def __init__(self, *args, **kwargs):
		...
		btnexplorer = Button(self.frame2, image=self.imgexplore, relief='flat', command=lambda: Zeta.Utility.Launch.Explorer(home=Zeta.System.Path.Scraps.path, geometry=Workspace.geometry['sidebar'], transient=Workspace.panel[Workspace.active]['root']))
		btnexplorer.pack(side='top', fill='x')
		btnexplorer.bind('<Button-3>', lambda e: Zeta.Utility.Launch.Explorer(home=Zeta.System.Path.Scraps.path, geometry=Workspace.geometry['main'], transient=Workspace.panel[Workspace.active]['root']))
		btneditor = Button(self.frame2, image=self.imgtext, relief='flat', command=lambda: Zeta.Utility.Launch.Editor(path=f'<Scraps>/workspace/void/{Workspace.active}/{str(round(time.time()))}.txt', geometry=f"+{25+333+10}+25", transient=Workspace.panel[Workspace.active]['root']))
		btneditor.pack(side='top', fill='x')
		btneditor.bind('<Button-3>', lambda e: Zeta.Utility.Launch.Editor(path=f'<Scraps>/workspace/void/{Workspace.active}/{str(round(time.time()))}.txt', geometry=Workspace.geometry['main'], transient=Workspace.panel[Workspace.active]['root']))
